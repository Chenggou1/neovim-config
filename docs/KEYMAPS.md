...
| 快捷键 | 模式 | 功能                        |
| ------ | ---- | --------------------------- |
| `jk`   | Insert | 退出到 Normal 模式（保留 ESC 用于其他工具） |
| `jk`   | Terminal | 退出到 Normal 模式（保留 ESC 用于终端程序） |

💡 **提示**：
- 统一使用 `jk` 快速组合键退出编辑/终端模式
- ESC 键保留原生功能，可供 Codex、Vim 插件等终端程序使用
- 需要快速连按 `j` 和 `k`（约 0.5 秒内）才会触发
...
| `H`         | 跳到行首（非空白）          |
| `L`         | 跳到行尾                    |
| `<leader><Left>`  | 返回上一位置（类 VSCode）   |
| `<leader><Right>` | 前进到下一位置（类 VSCode） |

### 复制历史与系统剪贴板

| 快捷键       | 模式          | 功能                       |
| ------------ | ------------- | -------------------------- |
| `y`          | Normal/Visual | 复制并加入 Yanky 历史      |
| `p` / `P`    | Normal/Visual | 在光标后/前粘贴            |
| `<C-p>`      | Normal        | 切换到上一条复制历史       |
...
- `p` 后可连续按 `<C-p>` / `<C-n>`，直接替换刚粘贴的内容
- Yanky 历史和系统剪贴板相互独立，避免系统剪贴板中的大量内容混入列表
...
| `ZZ`   | 保存当前窗口并退出 | `:wq` |
| `ZQ`   | 不保存当前窗口直接退出 | `:q!` |
| `:wqa` | 保存所有文件并正常退出 Neovim/Neovide | `:wqa` |
| `:qa!` | 放弃未保存内容并强制退出全部窗口 | `:qa!` |

💡 **提示**：

- Neovide 中推荐使用 `:wqa`，这样会正常触发会话保存
- `Cmd+Q` 可以关闭 Neovide，但有未保存文件时优先使用 `:wqa`
...
- **快速复制**：`<A-S-j>` 保留原行并在下方生成副本，Visual 模式下会复制整个选区
- 所有操作支持撤销合并（多次移动算作一次撤销）
- macOS 可能将部分 `Option+Shift` 组合转换为特殊字符；遇到这种情况可使用 Vim 原生的 `yyp`
...
| 快捷键 | 功能 |
|--------|------|
| `jk` | 退出终端模式（进入 Normal 模式） |
| `q` | 在终端 Normal 模式中隐藏当前终端 |
| `<C-h>` | 切换到左侧窗口 |
| `<C-l>` | 切换到右侧窗口 |

💡 **提示**：
- ESC 键保留原生功能，可正常用于 Codex、Vim、less 等终端程序
- 在终端中运行交互式程序时，ESC 键会正常传递给程序而不会退出终端模式
- `<leader>tn` 会用当前项目/目录名自动命名终端，名称最长 32 字符，同名时追加 `-2`、`-3`
- `<leader>tf` 会使用同样的命名规则，并追加 `-float` 后缀
- ToggleTerm 支持用编号切换终端，例如 `2<C-\>` 打开/隐藏 2 号终端
...
| `<leader>k` | Coding Agent（Sidekick/Codex）      |

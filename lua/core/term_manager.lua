...
-- 终端类型定义
local TERM_TYPES = {
    term1 = { slot = 1, direction = "horizontal" },
    term2 = { slot = 2, direction = "horizontal" },
    float_term = { slot = 3, direction = "float" },
}
...
-- 获取或创建指定类型的终端
function M.get_or_create_terminal(term_type)
...
function M.toggle_terminal(term_type, resolve_cwd_fn)
    local term = M.get_or_create_terminal(term_type)

    -- 更新工作目录
    if resolve_cwd_fn then
        term.dir = resolve_cwd_fn()
    end
